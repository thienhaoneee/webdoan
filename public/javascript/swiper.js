function usingSwiper(numberSection) {
    ...
    const swiperItems = document.querySelectorAll(
        ".single-product__swiper-item"
    );
    if(Array.from(swiperItems).length <= 1) return;
    ...
    function changeSwiper(swiperIndex) {
        ...
        swiperItems[swiperIndex].classList.add(
            "single-product__swiper-item--active"
        );
    }
    ...
    const getSlides = () => {
        return slider.querySelectorAll(".slider-item");
    };
